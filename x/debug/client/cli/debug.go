...
func CmdSetLogLevel(queryRoute string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "set-loglevel",
		Args:  cobra.ExactArgs(1),
		Short: "Set the zenchaind log level",
		Long: strings.TrimSpace(`
$ zenchaincli debug set-loglevel "main:info,state:info"

$ zenchaincli debug set-loglevel "upgrade:error"
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			if len(args) > 1 {
			...
			fmt.Println("Succeed to set the zenchaind log level.")
			return nil
		},
	}
}
