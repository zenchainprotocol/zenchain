...
import (
	...
	dexUtils "github.com/zenchainprotocol/zenchain-node/x/dex/client/utils"
...
$ zenchaincli tx dex list --base-asset mytoken --quote-asset okt --from mykey
`),
...
$ zenchaincli tx dex deposit mytoken_okt 1000okt --from mykey
...
$ zenchaincli tx dex withdraw mytoken_okt 1000okt --from mykey
...
$ zenchaincli tx dex register-operator --website http://xxx/operator.json --handling-fee-address addr --from mykey
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
...
$ zenchaincli tx dex edit-operator --website http://xxx/operator.json --handling-fee-address addr --from mykey
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
